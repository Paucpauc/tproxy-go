...
# Build DEB package for tproxy using minimal Docker image
# Uses alpine-based image with only dpkg installed
# Much lighter than debian:bullseye with dpkg-dev
...
echo "Building DEB package for $ARCH using minimal approach..."
...
# Build DEB package using minimal Docker image
echo "Building DEB package using minimal Docker image..."
docker run --rm \
    ...
    sh -c "
        ...
        dpkg-deb --build . /build/tproxy_1.0.0_${DEB_ARCH}.deb
    "
...
echo "Minimal DEB package build completed successfully!"
echo "Image used: alpine:latest (vs debian:bullseye)"
echo "Packages installed: dpkg only (vs dpkg-dev + dependencies)"